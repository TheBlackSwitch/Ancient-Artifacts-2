...
execute if data entity @s attack unless entity @e[tag=break,distance=..0.6] run summon item_display ~ ~0.5 ~ {item:{id:"minecraft:gunpowder",Count:1b,components:{"minecraft:item_model":"ancient_artifacts:item/air"}},Tags:[break, smithed.entity, smithed.strict]}
...
execute if data entity @s attack if score @s chest_break_progress matches 4 run kill @e[type=interaction,tag=artifact_chest,distance=..0.7]
