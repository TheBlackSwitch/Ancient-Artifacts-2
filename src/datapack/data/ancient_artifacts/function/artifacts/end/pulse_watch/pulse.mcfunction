...
summon item_display ~ ~0.01 ~ {transformation:{left_rotation:[0f,0f,0f,1f],right_rotation:[0f,0f,0f,1f],translation:[0f,0f,0f],scale:[20f,1f,20f]},interpolation_duration:10,Tags:["pulse","INIT",smithed.entity,smithed.strict],item:{id:"minecraft:gunpowder",count:1,components:{"minecraft:item_model":"ancient_artifacts:misc/pulse"}}}
...
$execute as @e[type=!#ancient_artifacts:no_damage,tag=!tamed,predicate=!theblackswitch:__version__/__patch__/player_id/match_search,distance=..$(distance),tag=!smithed.strict] run function ancient_artifacts:utilities/damage_entity_by_search {damage:4}
$execute as @e[tag=!tamed,predicate=!theblackswitch:__version__/__patch__/player_id/match_search,distance=..$(distance),tag=!smithed.strict] at @s run function ancient_artifacts:artifacts/end/pulse_watch/pull_entity
