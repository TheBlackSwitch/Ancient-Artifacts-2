summon drowned ~ ~-2 ~ {Tags:["necromanced","entity","INIT","tamed",smithed.entity],DeathLootTable:"ancient_artifacts:empty",equipment:{head:{id:"minecraft:gunpowder",count:1,components:{"minecraft:item_model":"ancient_artifacts:entity/sculk_mob"}}},drop_chances:{head:0}, \
    active_effects:[ \
...
scoreboard players operation @n[type=drowned,tag=necromanced,tag=entity,tag=INIT,distance=..3] tbs.ID = @s tbs.ID
...
tag @e[type=drowned,tag=necromanced,tag=entity,tag=INIT,distance=..3] remove INIT
