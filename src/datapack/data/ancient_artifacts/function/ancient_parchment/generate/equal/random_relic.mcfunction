# gen rand nbr used as an index to select a relic
...
# try a max of 1000 times
scoreboard players add #loop_count temp 1 
execute if score #loop_count temp matches 1000.. run return fail

# verify the relic
return run function ancient_artifacts:ancient_parchment/generate/equal/verify_relic with storage ancient_artifacts:parchment
