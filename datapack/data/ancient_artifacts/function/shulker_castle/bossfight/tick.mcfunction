...
execute if score .10 slow_tick matches 5 as @a[scores={shulker_king_music_timer=1..}] at @s unless entity @e[type=marker,tag=shulker_castle,distance=..63,tag=boss_started,tag=!boss_finished] run tag @s remove shulker_king_music
execute if score .10 slow_tick matches 5 as @a[scores={shulker_king_music_timer=1..}] at @s unless entity @e[type=marker,tag=shulker_castle,distance=..63,tag=boss_started,tag=!boss_finished] run function ancient_artifacts:shulker_castle/bossfight/song/reset

#Healer
execute if score .60 slow_tick matches 60 as @e[type=shulker,tag=healing_shulker] at @s run function ancient_artifacts:shulker_castle/bossfight/shulker_king/attacks/summon_healer/healer
