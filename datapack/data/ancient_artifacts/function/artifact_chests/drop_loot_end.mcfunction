...
execute store result storage ancient_artifacts:item_motion X double 0.01 run random value -5..5
execute store result storage ancient_artifacts:item_motion Z double 0.01 run random value -5..5
execute store result storage ancient_artifacts:item_motion Y double 0.01 run random value 5..15

##Set motion value based on the direction of the chest
execute align xyz if entity @e[y_rotation=90,type=item_display,dy=0] run data modify storage ancient_artifacts:item_motion X set value -0.3d
execute align xyz if entity @e[y_rotation=0,type=item_display,dy=0] run data modify storage ancient_artifacts:item_motion Z set value 0.3d
execute align xyz if entity @e[y_rotation=-90,type=item_display,dy=0] run data modify storage ancient_artifacts:item_motion X set value 0.3d
execute align xyz if entity @e[y_rotation=180,type=item_display,dy=0] run data modify storage ancient_artifacts:item_motion Z set value -0.3d

function ancient_artifacts:artifact_chests/summon_item with storage ancient_artifacts:item_motion
...
##Artifact particle
execute if score .artifact temp matches 1.. run particle firework ~ ~ ~ 0.0 0.0 0.0 0.3 10 force
