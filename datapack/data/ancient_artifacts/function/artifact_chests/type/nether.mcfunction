...
execute if data entity @s[tag=!opened] interaction on target unless entity @s[nbt={SelectedItem:{id:"minecraft:amethyst_shard",components:{"minecraft:custom_data":{compound:1b,type:"nether"}}}}] run title @s actionbar {"text": "It seems like some sparkling crystal is needed...","color": "light_purple"}
