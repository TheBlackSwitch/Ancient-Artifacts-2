...
execute if score .10 slow_tick matches 10 store result score .1 temp if entity @a[tag=deactivated,distance=..20]
execute if score .10 slow_tick matches 10 if score .1 temp matches 3.. run advancement grant @s only ancient_artifacts:artifacts/overworld/deactivation_expert