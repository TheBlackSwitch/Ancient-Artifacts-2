summon wither_skeleton ~ ~-2 ~ {Tags:["necromanced","entity","INIT","tamed"],DeathLootTable:"minecraft:empty",equipment:{mainhand:{id:"stone_sword",components:{unbreakable:{}}},head:{id:"minecraft:gunpowder",count:1,components:{"minecraft:item_model":"ancient_artifacts:entity/sculk_mob"}}},drop_chances:{head:0,mainhand:0}, \
    active_effects:[ \
...
scoreboard players operation @n[type=wither_skeleton,tag=necromanced,tag=entity,tag=INIT,distance=..3] tbs.ID = @s tbs.ID
...
tag @e[type=wither_skeleton,tag=necromanced,tag=entity,tag=INIT,distance=..3] remove INIT
