...
execute if score @s lifetime matches ..40 unless entity @s[type=drowned] at @n[type=marker,tag=necromanced,tag=particles,predicate=theblackswitch:matches_search_id] run particle block{block_state:"sculk"} ~ ~ ~ 0.6 0.1 0.6 0.1 100 force
execute if score @s lifetime matches ..40 unless entity @s[type=drowned] at @n[type=marker,tag=necromanced,tag=particles,predicate=theblackswitch:matches_search_id] run particle minecraft:sculk_charge_pop ~ ~ ~ 0.6 0.1 0.6 0 10 force
...
execute if score @s lifetime matches 41 unless entity @s[type=drowned] as @n[type=marker,tag=necromanced,tag=particles,predicate=theblackswitch:matches_search_id] run kill @s

scoreboard players add @s lifetime 1

execute if score @s lifetime matches ..40 run return fail

##Check if (real) entity is allive, else kill
...
execute on vehicle at @s if score .18 slow_tick matches 16.. store result storage ancient_artifacts:attack damage float 1 on vehicle run attribute @s attack_damage get
execute if score .18 slow_tick matches 16.. on target positioned ~ ~-2 ~ if entity @s[distance=0.01..1.5,tag=!damager] run function ancient_artifacts:artifacts/deep_dark/necromancy_anklet/damage with storage ancient_artifacts:attack
