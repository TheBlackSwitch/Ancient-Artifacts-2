...
execute if score @s sneak matches 1.. if score @s breath_power matches 60.. rotated ~ 0 positioned ^ ^ ^4 if score .10 slow_tick matches 10 run function ancient_artifacts:artifacts/end/dragons_breath_circlet/summon

scoreboard players operation .search tbs.ID = @s tbs.ID

function ancient_artifacts:tag_tamed

execute if score @s sneak matches 1.. if score @s breath_power matches 60.. rotated ~ 0 positioned ^ ^ ^2 as @e[tag=!tamed,type=!#ancient_artifacts:no_damage,tag=!no_damage,predicate=!theblackswitch:matches_search_id,distance=..2] run function ancient_artifacts:utilities/damage_entity_by_search {damage:3}
