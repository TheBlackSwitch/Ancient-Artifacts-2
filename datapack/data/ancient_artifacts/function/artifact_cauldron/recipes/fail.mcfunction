...
execute if score @s animation matches 77 run playsound entity.wither.shoot block @a ~0.5 ~0.5 ~0.5 .3 1
execute if score @s animation matches 80 run summon block_display ~.625 ~.7 ~.625 {brightness:{sky:15,block:15},transformation:{left_rotation:[0f,0f,0f,1f],right_rotation:[0f,0f,0f,1f],translation:[-0.5f,0f,-0.5f],scale:[0.75f,0.75f,0.75f]},block_state:{Name:"minecraft:fire"},Tags:[fire,artifact_cauldron]}
...
execute if score .3 slow_tick = .3 random_delay if score @s animation matches 1..10 run loot spawn ~0.5 ~0.5 ~0.5 loot ancient_artifacts:cauldron/fail
execute if score .3 slow_tick = .3 random_delay if score @s animation matches 1..10 as @e[type=item,distance=..1] run data modify entity @s Motion[1] set value 0.3d
execute if score .3 slow_tick = .3 random_delay if score @s animation matches 1..10 as @e[type=item,distance=..1] run tag @s add smoking
