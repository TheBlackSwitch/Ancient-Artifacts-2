...
execute on target if items entity @s weapon.mainhand *[custom_data~{compound: 1b, type: "end"}] run return run function ancient_artifacts:artifact_chests/end/interact/nested_return_0
playsound minecraft:block.amethyst_block.hit block @a ~ ~ ~ 1 2
execute on target run data modify storage smithed.actionbar:input message set value {json: {text: "It seems like a chorus compound is needed...", color: "light_purple"}, priority: "notification"}
