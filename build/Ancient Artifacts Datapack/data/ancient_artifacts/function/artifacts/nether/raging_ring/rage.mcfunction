...
execute on passengers run data modify entity @s angry_at set from entity @e[type=#ancient_artifacts:can_harm, tag=!rage_controller, tag=!smithed.strict, predicate=!theblackswitch:v2.0/patch-2/player_id/match_search, limit=1, sort=nearest, distance=..20] UUID
...
execute as @e[type=#ancient_artifacts:can_harm, tag=!smithed.strict, tag=!rage_controller, predicate=!theblackswitch:v2.0/patch-2/player_id/match_search, limit=1, sort=nearest, distance=..2] run function ancient_artifacts:artifacts/nether/raging_ring/damage with storage ancient_artifacts:attack
tag @s remove raged
