...
execute if entity @s[tag=mine_experience] if predicate theblackswitch:v2.0/patch-2/rand/10 align xyz positioned ~0.5 ~0.5 ~0.5 run function ancient_artifacts:artifacts/deep_dark/experienced_chain/summon_orb
...
execute if entity @s[tag=smelt] if entity @e[type=item, dy=0, tag=!checked_smelt] if score .is_smelted temp matches 1 run scoreboard players add @a[predicate=theblackswitch:v2.0/patch-2/player_id/match_search] blocks_smelted 1
