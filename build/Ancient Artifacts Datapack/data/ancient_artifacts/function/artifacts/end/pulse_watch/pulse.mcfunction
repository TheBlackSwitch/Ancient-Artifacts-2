...
$execute as @e[type=!#ancient_artifacts:no_damage,tag=!tamed,predicate=!theblackswitch:v2.0/patch-1/player_id/match_search,distance=..$(distance),tag=!smithed.strict] run function ancient_artifacts:utilities/damage_entity_by_search {damage:4}
$execute as @e[tag=!tamed,predicate=!theblackswitch:v2.0/patch-1/player_id/match_search,distance=..$(distance),tag=!smithed.strict] at @s run function ancient_artifacts:artifacts/end/pulse_watch/pull_entity
